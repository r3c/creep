...
class GitTracker:
    ...
    def current(self, base_path):
        revision = (
            Process(["git", "rev-parse", "--quiet", "--verify", "HEAD"])
    ...
    def diff(self, base_path, work_path, rev_from, rev_to):
        ...
        if rev_from is None or rev_from == "":
            ...
            self.logger.error(
                'Corrupted source revision "{0}" (must be a valid Git hash).'.format(
                    rev_from
                )
            )
        ...
        if not res_from:
            ...
            self.logger.error(
                'Unknown source revision "{0}" (must be a valid Git tree-ish).'.format(
                    rev_from
                )
            )
        ...
        res_to = (
            Process(["git", "rev-parse", "--quiet", "--verify", rev_to])
            .set_directory(base_path)
            .execute()
        )
        ...
        if not res_to:
            ...
            self.logger.error(
                'Unknown target revision "{0}" (must be a valid Git tree-ish).'.format(
                    rev_to
                )
            )
        ...
        if hash_from != hash_to:
            ...
            self.logger.info(
                "Update from revision ((fuchsia)){0}((default)) to ((fuchsia)){1}((default)).".format(
                    f, t
                )
            )
        else:
            self.logger.info(
                "Already at revision ((fuchsia)){0}((default)).".format(hash_from[0:8])
            )
        ...
        try:
            ...
            extract = (
                Process(["tar", "xf", temp_path]).set_directory(work_path).execute()
            )

            if not extract:
                self.logger.error(extract.err.decode("utf-8"))
                self.logger.error(
                    "Couldn't extract Git archive to temporary directory."
                )
