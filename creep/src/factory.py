...
def _detect_tracker(directory):
    ...
    if any(
        (
            os.path.exists(drive + os.path.join(*(names[0:n] + [".git"])))
...
def create_deployer(logger, connection, options, base_path):
    ...
    if match is None:
        logger.error(
            'Could not parse connection strings "{0}" as a URL.'.format(connection)
        )
    ...
    if scheme == "file":
        if password is not None or port is not None or user is not None:
            logger.warning(
                'Connection string for "file" scheme shouldn\'t contain any port, user or password.'
            )
    ...
    if scheme == "ftp" or scheme == "ftps":
        ...
        return FTPDeployer(
            logger, scheme == "ftps", host, port, user, password, directory, options
        )

    if scheme == "ssh":
        if password is not None:
            logger.warning(
                'Connection string for "ssh" scheme shouldn\'t contain any password.'
            )
