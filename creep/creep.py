...
def main():
    ...
    parser.add_argument(
        "--extra-append", action="append", default=[], help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--extra-remove", action="append", default=[], help=argparse.SUPPRESS
    )
    ...
    if not application.run(
        definition, args.names, append, remove, args.rev_from, args.rev_to
    ):
        return 1
