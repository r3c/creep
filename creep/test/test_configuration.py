...
class ConfigurationTester(unittest.TestCase):
    ...
    def test_get_orphan_keys(self, logger_mock):
        ...
        configuration.open_field("f1")
        configuration.open_field("f3")
    ...
    @mock.patch("logging.Logger")
    def test_open_field_incompatible(self, logger_mock):
    ...
    @mock.patch("logging.Logger")
    def test_open_field_valid(self, logger_mock):
        configuration = self.load_object({"f1": 42, "f2": "17"}, logger_mock)

        self.assertEqual(configuration.open_field("f1").read_value(int, None), 42)
        self.assertEqual(configuration.open_field("f2").read_value(str, None), "17")

        logger_mock.error.assert_not_called()
        logger_mock.warning.assert_not_called()

    @mock.patch("logging.Logger")
    def test_open_object_incompatible(self, logger_mock):
        configuration = self.load_object([], logger_mock)
        configuration.open_object()
    ...
    @mock.patch("logging.Logger")
    def test_open_object_valid(self, logger_mock):
        configuration = self.load_object({"k1": 1, "k2": "2", "k3": False}, logger_mock)
        configuration_object = configuration.open_object()
    ...
    @mock.patch("logging.Logger")
    def test_open_list_incompatible(self, logger_mock):
        configuration = self.load_object({}, logger_mock)
        configuration.open_list()
    ...
    @mock.patch("logging.Logger")
    def test_open_list_valid(self, logger_mock):
        configuration = self.load_object([1, "2", False], logger_mock)
        configuration_list = configuration.open_list()
